...
def get_small_report(request, spots, report_type):
    ...
    if value:
        ...
        if report_type == "weather":
            ...
            condition = "it s {}".format(report["condition"])
...
def get_full_report(request, spots):
    ...
    if value:
        ...
        forecast = "wind is {} ({}), with speed between {} and {}".format(report["directionType"], int(report["direction"]), report["speed"], report["gust"])
        messages.append({"text": {"text": [forecast]}})
        # weather
        weather = report.get_report("weather")
        forecast = "air temperature is {} and weather is {}".format(weather["temperature"], weather["condition"])
        messages.append({"text": {"text": [forecast]}})  
        # tides 
